...
4. Boss के काम को समझो
5. Sarcasm और humor का use करो

तुम सिर्फ एक assistant नहीं हो - तुम एक SYSTEM हो।"""
...
def get_gemini_response(user_input):
    try:
        ...
        quota_manager.increment_message_count()
        return reply
    except Exception as e:
        print(f"❌ Error: {e}")
        if "quota" in str(e).lower() or "429" in str(e):
            quota_manager.rotate_key()
            return "Quota rotate हुआ। फिर से try करते हैं!"
...
while running:
    ...
    if face_detected:
        if absent_timer > 50:
            voice_manager.speak("वेलकम बैक बॉस।")
        absent_timer = 0
        warning_given_1 = False
        warning_given_2 = False
        status_msg = "BOSS: ONLINE"
        ...
        if absent_timer > 60 and not warning_given_1:
            voice_manager.speak("बॉस कहाँ गए?")
            warning_given_1 = True
        if absent_timer > 200 and not warning_given_2:
            voice_manager.speak("वापस आओ!")
            warning_given_2 = True
        if absent_timer > 300:
            voice_manager.speak("शट डाउन।")
            time.sleep(2)
            running = False
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if mx < 250 and my < 250:
                response = get_gemini_response("मुझे motivate करो!")
                voice_manager.speak(response)
            elif mx > screen_w - 250 and my < 250:
                response = get_gemini_response("मेरी स्थिति क्या है?")
                voice_manager.speak(response)
            elif mx < 250 and my > screen_h - 250:
                response = get_gemini_response("कोई मजेदार बात बता!")
                voice_manager.speak(response)
            elif mx > screen_w - 250 and my > screen_h - 250:
                response = get_gemini_response("मैं क्या करूँ अभी?")
                voice_manager.speak(response)

pygame.quit()
chat_memory.save()
print("✅ Myra shutdown complete!")
